...
This repository contains three independent platform crates (`platform/dev-imxrt`, `platform/dev-mcxa`, `platform/dev-npcx`, `platform/dev-qemu`), each with its own `Cargo.lock` and its own `supply-chain/` store. Run all `cargo vet` commands from the relevant platform directory.
